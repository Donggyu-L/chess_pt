...
from env_PT import Chess
from mcts_Alphazero_PT import MCTSPlayer
from file_utils_PT import *
from network_PT2 import *
from collections import defaultdict, deque
from g4_reward import *
...
path = '/home/hail/PreferenceTransformer/JaxPref/reward_model/chess/PrefTransformer/new_cel_2/s5/'
...
def policy_evaluate(env, current_mcts_player, old_mcts_player, n_games=30):  # total 30 games
    ...
    for j in range(n_games):
        winner, move_len = start_play(env, training_mcts_player, opponent_mcts_player) # here?
        win_cnt[winner] += 1
...
if __name__ == '__main__':
    ...
    old_i2 = 215
    best_old_model, _ = create_models(n_playout, (old_i2 - 1))

    # policy_value_net = PolicyValueNet(state.shape[0], state.shape[1], device, best_old_model)
    policy_value_net = PolicyValueNet(state.shape[0], state.shape[1], device, model_file='Eval/nmcts25/train_215.pth')
    ...
    try:
        for i in range(215, training_iterations):
